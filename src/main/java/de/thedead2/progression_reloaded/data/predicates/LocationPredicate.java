...
public class LocationPredicate implements ITriggerPredicate<BlockPos> {
    ...
    public static LocationPredicate fromJson(JsonElement jsonElement) {
        if (jsonElement != null && !jsonElement.isJsonNull()) {
            ...
            ResourceKey<Level> levelKey = jsonObject.has("dimension") ? ResourceLocation.CODEC.parse(JsonOps.INSTANCE, jsonObject.get("dimension")).resultOrPartial(LOGGER::error).map((resourceLocation) -> ResourceKey.create(Registries.DIMENSION, resourceLocation)).orElse(null) : null;
            ResourceKey<Structure> structureKey = jsonObject.has("structure") ? ResourceLocation.CODEC.parse(JsonOps.INSTANCE, jsonObject.get("structure")).resultOrPartial(LOGGER::error).map((resourceLocation) -> ResourceKey.create(Registries.STRUCTURE, resourceLocation)).orElse(null) : null;
    ...
    public boolean matches(BlockPos blockPos, Object... addArgs) {
        ...
        if (!this.x.matches(blockPos.getX())) {
            ...
            return false;
        } else if(serverLevel == null) return true;
            ...
            if (this.biome == null || flag && serverLevel.getBiome(blockPos).is(this.biome)) {
                if (this.structure == null || flag && serverLevel.structureManager().getStructureWithPieceAt(blockPos, this.structure).isValid()) {
                    if (!this.block.matches(serverLevel.getBlockState(blockPos), serverLevel.getBlockEntity(blockPos))) {
                        return false;
                    } else {
    ...
    public JsonElement toJson() {
        if (this == ANY) {
            ...
            if (this.dimension != null) {
                Level.RESOURCE_KEY_CODEC.encodeStart(JsonOps.INSTANCE, this.dimension).resultOrPartial(LOGGER::fatal).ifPresent((jsonElement) -> jsonObject.add("dimension", jsonElement));
            }
    ...
    public static LocationPredicate from(BlockPos blockPos, Object... addArgs) {
        ...
        if(serverLevel != null){
            ...
            serverLevel.structureManager().getAllStructuresAt(blockPos).keySet().stream().findAny().ifPresent(structure -> structureKey.set(ResourceKey.create(Registries.STRUCTURE, serverLevel.registryAccess().registryOrThrow(Registries.STRUCTURE).getKey(structure))));
            ...
            return new LocationPredicate(MinMax.Doubles.exactly(x), MinMax.Doubles.exactly(y), MinMax.Doubles.exactly(z), biomeKey, structureKey.get(), dimensionKey, block, fluid);
        }
        return new LocationPredicate(MinMax.Doubles.exactly(x), MinMax.Doubles.exactly(y), MinMax.Doubles.exactly(z), null, null, null, BlockPredicate.ANY, FluidPredicate.ANY);
    }
