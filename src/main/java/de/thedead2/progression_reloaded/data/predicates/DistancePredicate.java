...
public class DistancePredicate implements ITriggerPredicate<DistancePredicate.DistanceInfo> {
    ...
    public static DistancePredicate fromJson(@Nullable JsonElement pJson) {
        if (pJson != null && !pJson.isJsonNull()) {
            ...
            return new DistancePredicate(MinMaxNumber$doubles, MinMaxNumber$doubles1, MinMaxNumber$doubles2, MinMaxNumber$doubles3, MinMaxNumber$doubles4);
        } else {
            return ANY;
        }
    }
    ...
    public static DistancePredicate from(DistanceInfo distanceInfo){
        if(distanceInfo == null) return ANY;
        float xDistance = distanceInfo.getxDistance();
        float yDistance = distanceInfo.getyDistance();
        float zDistance = distanceInfo.getzDistance();

        return new DistancePredicate(MinMax.Doubles.exactly(xDistance), MinMax.Doubles.exactly(yDistance), MinMax.Doubles.exactly(zDistance), MinMax.Doubles.ANY, MinMax.Doubles.ANY);
    }
