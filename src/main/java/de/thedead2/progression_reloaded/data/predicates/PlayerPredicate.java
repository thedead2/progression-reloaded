...
public class PlayerPredicate implements ITriggerPredicate<SinglePlayer> {
    ...
    public boolean matches(SinglePlayer player, Object... addArgs) {
        ...
        if(this.level != null && !player.hasProgressionLevel(this.level)){
            ...
            EntityHitResult entityhitresult = ProjectileUtil.getEntityHitResult(serverplayer.level, serverplayer, vec3, vec32, (new AABB(vec3, vec32)).inflate(1.0D), (entity) -> !entity.isSpectator(), 0.0F);
    ...
    public JsonElement toJson() {
        ...
        if(this.level != null) jsonObject.addProperty("level", this.level.getId().toString());
        if(this.team != null) jsonObject.addProperty("team", this.team.getId().toString());
        if (this.gameMode != null) jsonObject.addProperty("gamemode", this.gameMode.getName());
    ...
    public static PlayerPredicate from(SinglePlayer player) {
        ...
        Vec3 vec31 = serverPlayer.getViewVector(1.0F);
        Vec3 vec32 = vec3.add(vec31.x * LOOKING_AT_RANGE, vec31.y * LOOKING_AT_RANGE, vec31.z * LOOKING_AT_RANGE);
        EntityHitResult entityhitresult = ProjectileUtil.getEntityHitResult(serverPlayer.level, serverPlayer, vec3, vec32, (new AABB(vec3, vec32)).inflate(1.0D), (entity1) -> !entity1.isSpectator(), 0.0F);
        if (entityhitresult == null || entityhitresult.getType() != HitResult.Type.ENTITY) {
            entity = entityhitresult.getEntity();
        }

        return new PlayerPredicate(MinMax.Ints.exactly(experienceLevel), gameMode, player.getProgressionLevel(), player.getTeam().orElse(null), entity == null ? EntityPredicate.ANY : EntityPredicate.from(entity));
    }

    public static PlayerPredicate fromJson(JsonElement jsonElement){
        if(jsonElement == null || jsonElement.isJsonNull()) return ANY;
