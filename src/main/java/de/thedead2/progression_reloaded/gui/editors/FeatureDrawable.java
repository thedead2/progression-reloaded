...
import de.thedead2.progression_reloaded.api.gui.ICustomDrawGuiDisplay;
import de.thedead2.progression_reloaded.api.gui.ICustomDrawGuiEditor;
...
import de.thedead2.progression_reloaded.gui.core.IGuiFeature;
import de.thedead2.progression_reloaded.gui.editors.insert.FeatureNew;
import de.thedead2.progression_reloaded.gui.fields.BooleanField;
import de.thedead2.progression_reloaded.gui.fields.EnumField;
import de.thedead2.progression_reloaded.gui.fields.TextField;
...
import static de.thedead2.progression_reloaded.api.gui.Position.BOTTOM;
import static de.thedead2.progression_reloaded.api.gui.Position.TOP;
...
public abstract class FeatureDrawable<T extends IRuleProvider> extends FeatureAbstract {
    ...
    public FeatureDrawable(String text, int offsetY, IGuiFeature newDrawable, int gradient1, int gradient2, int fontColor, int color) {
        this.text = TextFormatting.BOLD + de.thedead2.progression_reloaded.ProgressionReloaded.translate("new." + text);
                ...
                if (MODE == EDIT && !FeatureNew.IS_OPEN) {
                    if (mouseX >= 1 && mouseX <= drawing.getWidth(MODE) - 16) {
                        if (mouseY >= yPos && mouseY < yPos + 6) {
                            ...
                            String translated = de.thedead2.progression_reloaded.ProgressionReloaded.translate(untranslated);
                            if (!("progression." + untranslated).equals(translated) || t.getFieldName().equals("isVisible") || t.getFieldName().equals("mustClaim") || t.getFieldName().equals("inverted")) {
                                if (t.getFieldName().equals("isVisible"))
                                    translated = de.thedead2.progression_reloaded.ProgressionReloaded.translate("isVisible");
                                if (t.getFieldName().equals("mustClaim"))
                                    translated = de.thedead2.progression_reloaded.ProgressionReloaded.translate("mustClaim");
                                if (t.getFieldName().equals("inverted")) translated = de.thedead2.progression_reloaded.ProgressionReloaded.translate("inverted");
