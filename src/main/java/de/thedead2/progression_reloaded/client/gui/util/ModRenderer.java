...
public abstract class ModRenderer {
    ...
    private static ItemStack extraLifeItem = null;

    private static float animationStartTime = 0;

    private static float animationTime = 1;

    private static float randomActivationOffsetX = 0;

    private static float randomActivationOffsetY = 0;
    ...
    public static void render(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
        ...
        renderLevelProgressOverlay();
        renderActiveQuestOverlay();
    }


    private static void renderExtraLifeAnimation(int guiWidth, int guiHeight, float partialTick) {
        if(animationStartTime > 0) {
            float timeLeft = animationTime - animationStartTime;
            float percentDone = (timeLeft + partialTick) / animationTime;
            float f1 = percentDone * percentDone;
            float f2 = percentDone * f1;
            float f3 = 10.25F * f2 * f1 - 24.95F * f1 * f1 + 25.5F * f2 - 13.8F * f1 + 4.0F * percentDone;
            ...
            //relocates starting pos to the given pX and pY pos
    ...
    private static void renderLevelProgressOverlay() {
        if(!ConfigManager.SHOULD_RENDER_OVERLAY.get()) {
        }

    }


    private static void renderActiveQuestOverlay() {
    }
