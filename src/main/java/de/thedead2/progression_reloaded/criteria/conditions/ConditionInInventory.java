...
public class ConditionInInventory extends ConditionBaseItemFilter implements ICustomDescription, IEnum, ISpecialFieldProvider, IStackSizeable {
    ...
    public String getDescription() {
        if (getProvider().isInverted()) return de.thedead2.progression_reloaded.ProgressionReloaded.format(getProvider().getUnlocalisedName() + ".description." + slotType.toString().toLowerCase() + ".inverted");
        else return de.thedead2.progression_reloaded.ProgressionReloaded.format(getProvider().getUnlocalisedName() + ".description." + slotType.toString().toLowerCase());
    }
