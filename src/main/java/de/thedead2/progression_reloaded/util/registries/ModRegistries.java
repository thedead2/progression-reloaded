...
public abstract class ModRegistries {
    ...
    public static void onMissingMappings(final MissingMappingsEvent event) {
        ...
        if(!missingLevels.isEmpty()) {
            ...
            if(isDevEnv()) {
                throw new IllegalStateException("Detected missing progression levels: " + missingLevels.stream().map(MissingMappingsEvent.Mapping::getKey).toList());
            }
        }
        ...
        if(!missingQuests.isEmpty()) {
            ...
            if(isDevEnv()) {
                throw new IllegalStateException("Detected missing progression quests: " + missingQuests.stream().map(MissingMappingsEvent.Mapping::getKey).toList());
            }
        }
