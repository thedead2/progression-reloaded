        ...
        findMatchingClasses(baseClass).stream()
                                      ...
                                      .forEach(aClass -> {
                                          if(!aClass.isAnnotationPresent(ExcludeFromEventBus.class)) {
                                              var list = Arrays.stream(aClass.getMethods())
                                                               .filter(method -> Modifier.isStatic(method.getModifiers()) && method.isAnnotationPresent(SubscribeEvent.class))
                                                               .toList();
                                              if(list.isEmpty()) {
                                                  throw new IllegalStateException("Class " + aClass.getName() + " doesn't provide a static @SubscribeEvent method! Annotate the class with @ExcludeFromEventBus if it uses some other form of event listening!");
                                              }
