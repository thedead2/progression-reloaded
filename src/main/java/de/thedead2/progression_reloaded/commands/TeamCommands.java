...
public class TeamCommands {
    ...
    public static void register() {
        ModCommand.Builder.newModCommand("teams/add/[name]", Map.of("[name]", StringArgumentType.string()), Collections.emptyMap(),
                                         context -> {
        ...
        ModCommand.Builder.newModCommand("teams/delete/[name]", Map.of("[name]", StringArgumentType.string()), Map.of("[team]", SUGGEST_TEAMS),
                                         context -> {
                                             var source = context.getSource();
                                             String name = StringArgumentType.getString(context, "name");
                                             ResourceLocation id = PlayerTeam.createId(name);
                                             ...
                                             PlayerDataHandler.getTeamData().ifPresent(teamData -> {
                                                 if(teamData.removeTeam(id)) {
                                                     ...
                                                     source.sendFailure(TranslationKeyProvider.chatMessage("unknown_team", ChatFormatting.RED, name));
                                                 }
                                             });
                                             return COMMAND_SUCCESS;
                                         }
        ...
        ModCommand.Builder.newModCommand("teams/[team]/add/[player]", Map.of("[team]", StringArgumentType.string(), "[player]", EntityArgument.players()), Map.of("[team]", SUGGEST_TEAMS),
                                         context -> {
                                             var source = context.getSource();
                                             String teamName = StringArgumentType.getString(context, "team");
                                             var team = PlayerDataHandler.getTeam(teamName);
        ...
        ModCommand.Builder.newModCommand("teams/[team]/remove/[player]", Map.of("[team]", StringArgumentType.string(), "[player]", EntityArgument.players()), Map.of("[team]", SUGGEST_TEAMS, "[player]", SUGGEST_MEMBERS),
                                         context -> {
                                             var source = context.getSource();
                                             String teamName = StringArgumentType.getString(context, "team");
                                             var team = PlayerDataHandler.getTeam(teamName);
        ...
        ModCommand.Builder.newModCommand("teams/[team]/members", Map.of("[team]", StringArgumentType.string()), Map.of("[team]", SUGGEST_TEAMS),
                                         context -> {
                                             var source = context.getSource();
                                             String teamName = StringArgumentType.getString(context, "team");
                                             var team = PlayerDataHandler.getTeam(teamName);
        ...
        ModCommand.Builder.newModCommand("teams/[team]/level", Map.of("[team]", StringArgumentType.string()), Map.of("[team]", SUGGEST_TEAMS),
                                         context -> {
                                             var source = context.getSource();
                                             String teamName = StringArgumentType.getString(context, "team");
                                             var team = PlayerDataHandler.getTeam(teamName);
